...
namespace Post.Cmd.Api.Controllers
{
    ...
    public class AddCommentController : ControllerBase
    {
        ...
        public async Task<ActionResult> AddCommentAsync(Guid id, AddCommentCommand command)
        {
            ...
            {
                _logger.Log(LogLevel.Warning, ex, "Couldn't retrieve aggregate, client passed an incorrect post Id targetting the aggregate!");
                return BadRequest(new BaseResponse
                {
                    Message = ex.Message,
                });
            ...
            {
                const string SAFE_ERROR_MESSAGE = "Error while processing request to add a comment of a post!";
